...
const renderSuccess_story_Page = (response) => {
  ...
  if (response.length === 0) {
    video_session.innerHTML = `<div class="col-12 text-center"><h5>No Video Lectures Found !</h5></div>`;
    if (video_session && video_session.parentElement) {
      const allSiblings = Array.from(video_session.parentElement.children).filter(
        (el) => el !== video_session
      );
