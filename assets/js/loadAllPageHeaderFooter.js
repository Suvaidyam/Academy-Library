...
(function () {
  ...
  function getHeaderElements() {
    ...
    headers.forEach((header) => {
      header.addEventListener("click", (e) => {
        ...
        if (
          clickedNav &&
          clickedNav.getAttribute("href") &&
          clickedNav.getAttribute("href").includes("#")
        ) {
  ...
  async function setAnnouncementslist() {
    ...
    const response = await fetch(
      `https://erp-ryss.ap.gov.in/api/method/get_announcement_list`,
    );
  ...
  async function set_calender_badge(params) {
    try {
      const response = await fetch(
        "https://erp-ryss.ap.gov.in/api/method/get_calendar_events",
      );
