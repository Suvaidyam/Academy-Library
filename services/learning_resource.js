...
const showSkeletons = () => {
  document.getElementById("lr-artifact-grid").innerHTML = Array(PAGE_SIZE)
    .fill('<div class="col-lg-4 col-md-6"><div class="lr-skeleton-card"></div></div>')
    .join("");
...
const buildArtifactCard = (item) => {
  const isVideo = item.category === "Video Lecture" && isVideoUrl(item.attachment);
  const grad    = thumbGradient(item.name || "");
  const icon    = catIcon(item.category);
  const fileUrl = resolveUrl(item.attachment);
  const title   = item.title || item.a_short_description_about_the_artifact || item.name;
...
const attachVideoHover = () => {
  document.querySelectorAll("#lr-artifact-grid .lr-artifact-card:not(.no-video)").forEach((card) => {
...
const renderPagination = (total, page, limit) => {
  ...
  pages.push(pageBtn(page - 1, '<i class="bi bi-chevron-left"></i>', page === 1));
  ...
  pages.push(pageBtn(page + 1, '<i class="bi bi-chevron-right"></i>', page === totalPages));
...
const goToPage = (page) => {
  fetchPage(page);
  document.getElementById("lr-artifact-grid")?.scrollIntoView({ behavior: "smooth", block: "start" });
};
  ...
  container.querySelectorAll(".lr-cat-card").forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (activeCategory === key) {
        ...
        activeCategory = key;
        container.querySelectorAll(".lr-cat-card").forEach((c) => c.classList.remove("active"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("lr-reset-btn")?.addEventListener("click", () => {
    ...
    if (si) si.value = "";
    document.querySelectorAll("#lr-category-cards .lr-cat-card").forEach((c) =>
