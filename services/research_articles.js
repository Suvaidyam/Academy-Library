// Research Articles tab (pages/research-library.html → #videos)
// Fetches ONLY Internal research articles (Knowledge Artifact doctype,
// category = "Article", artifact_source = "Internal") from the
// `get_research_articles` server script — see
// server_scripts/get_research_articles.py for the API.
//
// Structured the same way as "Libray Service/fsc_curriculum_sessions.js"
// (Course Content tab): FrappeApiClient for the call, a plain `state` object
// for the active filters, and numbered pagination with ellipsis.
...
async function fetchArticles(page = 1) {
  ...
  try {
    const params = {
      page,
      page_size: PAGE_SIZE,
      // Always sent explicitly — this tab must only ever show Internal
