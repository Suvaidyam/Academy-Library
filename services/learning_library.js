      ...
      liveCources.forEach((post) => {
        if (post) {
          ...
          newCard.querySelector(".blog-title").textContent =
            post.title || "No Title";
          newCard.querySelector(".short_introduction").textContent =
            post.short_introduction || "Unknown";
          newCard.querySelector(".published_on").textContent =
            "Published Date : " + post.published_on || "No Date";
      ...
      newCources.forEach((post) => {
        if (post) {
          ...
          newCard2.querySelector(".blog-img-2").src = post.image
            ? ENV.API_BASE_URL + post.image
            : "assets/img/blog/default.jpg";
          newCard2.querySelector(".post-category-2").textContent =
            post.category || "Uncategorized";
          newCard2.querySelector(".blog-title-2").textContent =
            post.title || "No Title";
          newCard2.querySelector(".short_introduction-2").textContent =
            post.short_introduction || "Unknown";
          newCard2.querySelector(".published_on-2").textContent =
            "Published Date : " + post.published_on || "No Date";
...
export async function getandSetDyanamicCourseDetailsAndName() {
  ...
  document.getElementById("courseTypeName").innerHTML = courseCategoryType
    ? courseCategoryType
    : "Courses";
  let ugCourseContent =
    "The Academy is currently offering one course under this program. The under graduation course is delivered through experiential learning. This program includes classroom sessions, field classes, online classes, and field practice with strong emphasis on learning by doing. The graduation course follows a multiple entry-exit policy, allowing students to exit after each year, if necessary, and continue the course later through earned credits.​";
  let pgCourseContent =
    "Academy is currently offering a two-year post graduation course that is earned through experiential learning.  This experiential learning program is conducted through classroom sessions, field classes, online classes, and field practice with much emphasis on learning by doing.​";
  let CertificationContent =
    "The Academy is currently offering one course under this program. The under graduation course is delivered through experiential learning. This program includes classroom sessions, field classes, online classes, and field practice with strong emphasis on learning by doing. The graduation course follows a multiple entry-exit policy, allowing students to exit after each year, if necessary, and continue the course later through earned credits.​";
  if (courseCategoryType == "Certificate") {
    document.getElementById("courseContentsDetails").innerHTML =
