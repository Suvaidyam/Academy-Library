...
export async function getLibraryList() {
  try {
    ...
    async function knowledge_data() {
      let response = await frappe_client.get(
        "/get_knowledge_artificates",
        filter,
      );
    ...
    async function show_data() {
      ...
      if (selectedCategory && selectedCategory !== "Select Category") {
        filteredArtifacts = artifacts.filter(
          (artifact) => artifact.category === selectedCategory,
        );
      }
    ...
    let fieldMetaResponse = await frappe_client.get(
      "/get_field_meta",
      fieldMetaParams,
    );
    ...
    dropdownOptionsauthor += AuthorResponse.message?.emp
      ?.map(
        (item) =>
    ...
    dropdownOptions += fieldlinkResponse.message?.lang
      ?.map(
        (item) =>
    ...
    const availableTags = fieldtagsResponse.message?.tag?.map(
      (tag) => tag.tag_name,
    );
...
function displayArtifacts(filteredArtifacts) {
  ...
  if (cat && cat !== "Select Category") {
    artifactsToDisplay = filteredArtifacts.filter(
      (artifact) => artifact.category === cat,
    );
  }
  ...
  if (cat == "Article") {
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".blog-title").textContent =
          post.title || "No Title";
        newCard.querySelector(".articles_pdf").href =
          `${ENV.API_BASE_URL}${post.attachment}` || "#";
    ...
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".journal_details").textContent =
          post.a_short_description_about_the_artifact || "Uncategorized";
        newCard.querySelector(".blog-title").textContent =
          post.title || "No Title";
        newCard.querySelector(".post-author").textContent =
          post.internalauthor_name.employee_name || "Unknown";
        newCard.querySelector(".post-date").textContent =
          post.date_of_creationpublication || "No Date";
        newCard.querySelector(".journals_pdf").href =
          `${ENV.API_BASE_URL}${post.attachment}` || "#";
    ...
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".blog-img").src = post.thumbnail_image
          ? ENV.API_BASE_URL + post.thumbnail_image
          : "https://www.k12digest.com/wp-content/uploads/2024/03/1-3-550x330.jpg";
        newCard.querySelector(".case_studies_details").textContent =
          post.a_short_description_about_the_artifact || "Uncategorized";
        newCard.querySelector(".blog-title").textContent =
          post.title || "No Title";
        newCard.querySelector(".case_studies_pdf").href =
          `${ENV.API_BASE_URL}${post.attachment}` || "#";
    ...
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".blog-img").src = post.thumbnail_image
          ? ENV.API_BASE_URL + post.thumbnail_image
          : "https://www.k12digest.com/wp-content/uploads/2024/03/1-3-550x330.jpg";
        newCard.querySelector(".book_details").textContent =
          post.a_short_description_about_the_artifact || "Uncategorized";
        newCard.querySelector(".blog-title").textContent =
          post.title || "No Title";
        newCard.querySelector(".post-author").textContent =
        ...
        newCard.querySelector(".post-date").textContent =
          post.date_of_creationpublication || "No Date";
        newCard.querySelector(".books_pdf").href =
          `${ENV.API_BASE_URL}${post.attachment}` || "#";
    ...
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".newsletter_details").textContent =
          post.a_short_description_about_the_artifact || "Uncategorized";
        // newCard.querySelector(".blog-title").textContent = post.title || "No Title";
        newCard.querySelector(".newsletter_pdf").href =
          `${ENV.API_BASE_URL}${post.attachment}` || "#";
    ...
    artifactsToDisplay.forEach((post) => {
      if (post) {
        ...
        newCard.querySelector(".post-category").textContent =
...
c_dropdown.addEventListener("change", async function () {
  ...
  const filter = {
    ...
    ...(languageDropdown.value &&
      languageDropdown.value !== "Select Language" && {
        language: languageDropdown.value,
      }),
    ...(authorDropdown.value &&
      authorDropdown.value !== "Select Author" && {
        authorDropdown: authorDropdown.value,
      }),
    ...(yearDropdown.value &&
      yearDropdown.value !== "Select Year" && { year: yearDropdown.value }),
...
authorDropdown.addEventListener("change", async function () {
  ...
  const response = await frappe_client.get(
    "/get_knowledge_artificates",
    filter,
  );
  let totalCount = response.message.total_count;
...
const getLanguageList = async () => {
  ...
  try {
    ...
    const langCodes = [
      ...new Set(response.message.map((item) => item.language).filter(Boolean)),
    ];
