...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (response && response.message.data) {
      ...
      const remaining_stories = storiesList.filter(
        (item) => item.name !== storiesId,
      );
...
function populatestoriesDetails(stories) {
  ...
  imgElement.src =
    ENV.API_BASE_URL + stories.thumbnail_image ||
    "../assets/img/default-stories.jpg";
...
function set_remaining_stories(remaining_stories) {
  if (remaining_stories && remaining_stories.length > 0) {
    const remaining_stories_container =
      document.getElementById("remaining_stories");
    ...
    recentStories.forEach((item) => {
      ...
      const imageUrl = item.thumbnail_image
        ? `${ENV.API_BASE_URL}${item.thumbnail_image}`
        : "../assets/img/default-stories.jpg";
